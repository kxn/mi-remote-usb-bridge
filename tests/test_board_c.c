...
uint8_t EEPROM_ERASE(uint32_t a,uint32_t n){assert(a>=STORE_BASE&&a+n<=BLE_SNV_ADDR);assert(!(a%256)&&n==256);memset(nv+a,255,n);return 0;}
uint8_t EEPROM_WRITE(uint32_t a,void*p,uint32_t n){
    assert(a>=STORE_BASE&&a+n<=BLE_SNV_ADDR);
    if(writes++==fail_write){if(partial>n)partial=n;memcpy(nv+a,p,partial);return 1;}
    memcpy(nv+a,p,n);return 0;
}
static void reboot(void){loaded=false;current_slot=-1;storage_state=0;staged_valid=false;memset(current,0,sizeof current);}
int main(void){
