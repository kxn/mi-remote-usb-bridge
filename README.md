...
**已发布固件：0.6.6，协议 RBP/3.0。** 已在 CH582F + RC003 + Windows 上实测配对、按键、语音及 release 错误记录。此前版本已验证长录音和掉线重连；不是每个版本都重跑了全部射频场景。macOS 有串口及动作注入实现，尚未完成真机验收。
...
当前源码为 **0.6.14**（RBP/3 不变），新增持久化 GATT 缓存、联通语音预收、有界快速重连及更完整的底层错误记录。主机回归测试与 debug/release 编译通过；0.6.13-debug 已实测快速重试，0.6.14 release 尚未完成实机回归。联通首次建联仍可能出现 0x3E，录音分片连续性异常仍待定位，不能视为已修复。过程与验证边界见 [重连记录](docs/reconnect-0.6.13.md)；最新源码固件请按构建指南编译，以下预编译下载仍为 0.6.6。

## 功能与边界

维护者可通过 [GitHub Actions 发版](docs/github-release.md)，手动指定版本号或留空自动生成，构建 non-debug 固件并发布 Release。
