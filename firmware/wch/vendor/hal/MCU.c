/* Modified by Mi Remote USB Bridge: flash status propagation and SDK diagnostics.
 * See docs/vendor-patches.md. Original file encoding preserved. */
...
static void bridge_sdk_status(uint8_t code, uint32_t status)
{
    rbp_fault_record(RBP_FAULT_SDK,code,status,0);
#ifdef RBP_SDK_RX_PROBE
    uint32_t checks, crc;
    sdk_rx_probe_counts(&checks, &crc);
    DT(DT_SYS,DT_ERROR,4,code,status,checks,crc);
#else
    DT(DT_SYS,DT_ERROR,4,code,status,0,0);
#endif
}
...
{
    uint32_t status=EEPROM_READ(addr,pBuf,num*4);
    DT(DT_STORE,status?DT_ERROR:DT_RAW,9,addr,num*4,status,0);
    return status;
}
...
{
    uint32_t status = EEPROM_ERASE(addr, num * 4);
    DT(DT_STORE,status?DT_ERROR:DT_DETAIL,11,addr,num*4,status,0);
    if (status != 0) return status;
    status=EEPROM_WRITE(addr,pBuf,num*4);
    DT(DT_STORE,status?DT_ERROR:DT_DETAIL,12,addr,num*4,status,0);
    return status;
}
#endif
