...
class AudioDecoder:
    ...
    def format(self,fmt):
        ...
        try:
            ...
            self.decoder=ImaDecoder(fmt);self.fmt=fmt
        except ValueError:
            self.failed=True;self.buffer.clear();raise

    def data(self,c):
        self._ready()
        try:
            self.check.feed(c)
            # This relation belongs only to the current IMA decoder profile.
            if c.unit_sample_count!=UNKNOWN_COUNT and c.unit_sample_count!=c.unit_size*2:
    ...
    def end(self,event):
        ...
        finally:self.failed=True;self.buffer.clear()
