...
public sealed class AudioDecoder : IDisposable
{
    ...
    private NativeIco? _ico;
    public static (uint Codec,ushort Revision)[] SupportedCodecs=>NativeIco.Available?[(1,1),(2,1)]:[(1,1)];
    public void Dispose(){_ico?.Dispose();_ico=null;_failed=true;_buffer=[];}
    ...
    public void SetFormat(AudioFormat f)
    {
        ...
        try
        {
            if(f.Stream==0||f.Epoch!=_epoch+1||f.FirstUnit!=_unit||f.FirstSample!=_samples||_offset!=0||
               (Format is not null&&f.Stream!=Format.Stream)||f.Revision!=1||f.Channels!=1||
               (f.Codec==1?(f.Rate is not (8000 or 16000)||f.MaxUnit is 0 or >65536||f.Config.Length!=4||f.Config[2]>88||f.Config[3]!=0):
                f.Codec!=2||f.Rate!=16000||f.MaxUnit!=40||f.Config.Length!=0))
                throw new InvalidDataException("Unsupported audio format or invalid boundary");
            _ico?.Dispose();_ico=null;
            if(f.Codec==2)_ico=new NativeIco();
            else {_predictor=BinaryPrimitives.ReadInt16LittleEndian(f.Config);_index=f.Config[2];}
            Format=f;_epoch=f.Epoch;
    ...
    public byte[] Feed(byte[] data)
    {
        ...
        try
        {
            ...
            if(Wire.U32(data,36)!=0||stream!=Format.Stream||frame!=_frame+1||epoch!=_epoch||unit!=_unit||offset!=_offset||
                ...
                (Format.Codec==2&&size!=40)||(samples!=uint.MaxValue&&samples!=(Format.Codec==2?320:size*2))||(_offset!=0&&(size!=_size||samples!=_unitSamples)))
            ...
            byte[] pcm=_ico is not null?_ico.Decode(_buffer):new byte[size*4];int p=0;
            if(_ico is null)for(int i=0;i<size;i++)for(int half=0;half<2;half++)
            {
